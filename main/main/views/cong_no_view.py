...
class CongNoView(ModelViewSet):
    ...
    def export_cong_no(self, request):
        ...
        if khach_hang_id:
            ds_khach_hang = CongNoKhachHangModel.objects.filter(id=khach_hang_id)
        else:
            ds_khach_hang = CongNoKhachHangModel.objects.all()
        ...
        for khach_hang in ds_khach_hang:
            ...
            worksheet = workbook.add_worksheet(khach_hang.name)
